...
Four roles: three follow a hierarchical permission model; the fourth (Display Wall User) is a separate role for the TV display device.
...
- Configure system parameters (train dwell time, message prompt text, message length limit and unit, auto-moderator word list, default placeholder image)
