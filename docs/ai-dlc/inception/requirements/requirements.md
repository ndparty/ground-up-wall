...
- **NFR-11** (clarified Update 04): The display wall must recover gracefully from a browser refresh. On refresh it restores the approved-submission list, the active display-override state (from the database), and the current server-authoritative playback position and play/pause state (held in memory by the server; see FR-24a). In Phase 1 this server state is in-memory and single-instance, so a server-process restart resets playback to the start in playing state.
...
| R-07 | Display wall refresh resilience | In Phase 1 the server holds playback position and play/pause state in memory and the display restores them on browser refresh (see FR-24a, revised Update 04). This state is single-instance and is lost if the server process restarts (playback then resets to the start in playing state). Cross-instance/persistent playback may be added in a future iteration. |
