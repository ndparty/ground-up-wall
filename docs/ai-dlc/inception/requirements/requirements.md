...
- The prompt text shall be configurable by an Admin via the system parameters panel. The default prompt text shall be: *"Share your National Day moment!"* (Update 04)
- **FR-02a** (reworded Update 04): The upload form shall display a concise data privacy notice before submission, informing participants that their name, message, photo, and optional social handle will be displayed on the photowall during the event, and that submission data (including photos) will be retained indefinitely by the organisers for social media use — if an Instagram handle is provided, the content may be posted on social media with tagging to their handle. The notice shall include a reference for contacting an organiser with questions. The tone shall follow DR-04 (warm, community-appropriate). The upload form shall include a **mandatory acknowledgment checkbox**. Submission shall be blocked by inline validation until all required fields — photo, message (within the configured length), submitter name, and the acknowledgment checkbox — are satisfied; when the participant attempts to submit with any of these missing, the form shall show a clear inline error for each (including a "please acknowledge the privacy notice" hint for the unchecked box) and shall not submit. The server shall independently reject any submission that does not include the acknowledgment. The checkbox label shall state that the participant has read and understood the privacy notice and posting guidelines.
...
- **FR-05**: The upload page must be accessible via a short URL. A QR code for the event URL is generated in-app from the display's auto-detected base URL (no external artifact needed): the display wall shows a top bar with the base URL as plain text, and a recurring "join the wall" QR cabin (see FR-22a) renders a system-themed QR encoding the base origin so attendees can scan from the screen (Update 05).
...
- **FR-13a**: An Admin-only system parameters page shall exist for viewing and modifying configurable system parameters:
  ...
  - **QR cabin interval** (Update 05): How often the recurring "join the wall" QR cabin appears, in cabin advances. Default: 15. `0` disables it. See FR-22a.
...
- **FR-13b** (Update 05): The Admin system parameters page shall provide two operational toggles for running the event safely:
  - **Event killswitch** (`system_killswitch_enabled`, default off): when on, all functionality is disabled except the login page and the Admin panel (and their APIs). Blocked HTML routes show a simple "event is currently offline" page; blocked APIs return HTTP 503. This is intended to take the public-facing service down cleanly before and after the event.
  - **Public uploads toggle** (`uploads_enabled`, default on): when off, the public upload page and the submission endpoint are disabled (HTTP 503 / "uploads are closed"), while the display wall and moderation continue to run. Enforced by a server middleware gate ahead of the routes; the display wall and moderation remain unaffected.
...
- **FR-18**: The train consists of multiple cabins; each cabin displays one approved submission (photo + message + name + optional social handle). The photo is shown in a square (1:1) window (Update 05); uploads are center-cropped to 1:1 on both the client preview and server normalization. The message/name/handle panel is a constant height so all cabins are uniform.
...
- **FR-20a** (Update 04; generator model Update 05): **Off-screen mutation invariant.** Any cabin that enters or leaves the train — a preview of a newly approved submission (FR-22), a recurring QR cabin (FR-22a), and the collapse/restore used during a jump (FR-24a) — shall enter strictly from beyond the right edge of the visible window and leave only after passing beyond the left edge, never changing the identity, content, or on-screen position of any cabin currently within the visible band (the centered cabin and the K cabins on each side). **Update 05**: this is now satisfied by construction — a server-authoritative generator emits one cabin at a time at the right edge (see FR-22), so there is no in-place node-list surgery within the visible band. Exception: when the total number of approved cabins is at or below the visible slot count (2K+1), the whole sequence is on-screen and an append is unavoidably visible.
- **FR-21**: Cabin display order follows the chronological canonical sequence — oldest approved submissions first, newest appended to the end — walked forward with wraparound. A newly approved submission is appended to the canonical sequence (permanent rotation) and additionally shown once as a near-term preview (FR-22).
- **FR-22**: New approved submissions are added to the rotation automatically after approval (real-time, no manual refresh). **Update 05**: a server-authoritative generator drives the display — each dwell tick it emits the next cabin at the right edge: the front of an ephemeral FIFO queue (a just-approved preview, or a QR cabin) if present, otherwise the next sequential canonical cabin. The full visible window is broadcast to all display sessions so they stay in lockstep and refresh/late-join restores exactly (FR-24a / NFR-11). Cabins are identified by submission id, so deleting a post never corrupts cabins currently on screen.
- **FR-22a** (Update 05): A recurring "join the wall" QR cabin shall appear at an admin-configurable interval (FR-13a, `qr_cabin_interval`; `0` disables). It uses the same FIFO mechanism as approved previews (enqueued every N cabin advances, skipped if one is already queued), and renders a system-themed QR code generated in the browser from the display's base origin, with a short invitation to scan and post.
...
- **NFR-07** (reworded Update 04): The upload form shall be completable on a single screen without navigating away — all inputs (photo, message, name, optional social handle, privacy acknowledgment) and the submit action shall be present on one page with minimal, clearly grouped steps so a participant can submit quickly from a phone.
...
- **NFR-11** (clarified Update 04; snapshot Update 07): The display wall must recover gracefully from a browser refresh. On refresh it restores the approved-submission list, the active display-override state (from the database), and the current server-authoritative playback position and play/pause state (see FR-24a). Playback snapshot is also written to `system_config.train_playback_state` and restored on server process restart when valid; invalid snapshots reset to cabin 1 in playing state.
