...
This document evaluates **hashtag ingest** (written FRs) and **outbound posting** to the organiser’s Instagram (stakeholder Q&A). Recommendation at the end.
...
- Permissions typically include `instagram_basic` plus Page-related scopes (`pages_read_engagement`, etc.)
...
**Fit with existing schema:** `submissions.source` and `source_metadata` already exist (`db/schema.sql`). Moderation source badge (FR-30) is a small UI addition.
...
- App Review for **`instagram_business_content_publish`** (and `instagram_business_basic`; older `instagram_content_publish` scopes deprecated Jan 2025)  
...
**Tagging submitters:** `user_tags` on container may work for @mentions if we have handles; API does not give us poster username from hashtag ingest anyway.
...
- **Meta Platform Terms:** Storing and re-displaying imported media on a photowall likely requires permitted use under Public Content Access (campaign / audience understanding). Confirm wording with organiser.  
- **FR-02a retention:** Participants already consent to organiser social use; Instagram-sourced content should use the **same** moderation + retention path.  
- **PII:** Hashtag API does not return username; reduces accidental PII from API but caption may contain names/handles.  
- **Manual fallback:** Aligns with Sharon’s “post on IG after the event” if API ingest fails — moderators export or screenshot approved items manually.
...
**For a single National Day event:** Start Meta app setup **immediately** if ingest is desired on event day. Otherwise plan **fallback-only** for event and ingest for a future event.
...
2. Is **automated hashtag ingest** required on event day, or is **post-event outbound posting** enough?  
...
- [requirements.update01.context.md](https://github.com/ndparty/ground-up-wall-sdd/blob/main/updates/requirements.update01.context.md) — stakeholder IG Q&A (SDD repo)
