...
sudo -u groundupwall bash -c 'set -a && source /opt/ground-up-wall/.env && set +a && deno task build'
```

Fresh **2.x** registers file routes at build time (`_fresh/server.js`). Without `deno task build`,
`prod.ts` starts but every path returns plain `Not Found`.
...
`prod.ts` loads `_fresh/server.js` (generated by `deno task build`). Re-run the build after every
`git checkout` of a release tag — `scripts/deploy.sh` does this automatically.
