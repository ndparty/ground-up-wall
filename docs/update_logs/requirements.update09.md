...
- Probe appended inside `.train-cabin__message-wrap` only (not as flex sibling in `.train-cabin__body`).
- CSS in `static/fit-text.css` and nonce baseline in `routes/_app.tsx`: `position: absolute; inset: 0; visibility: hidden`.
...
Helpers: `loginPageRedirect`, `roleHomeRedirect`, `requireRolePage` in `lib/auth/login_redirect.ts` and `lib/middleware/auth_guard.ts`.
...
- Logged in as `display`, visit `/moderate` → `/display`; as `moderator`, visit `/admin` → `/moderate`.
